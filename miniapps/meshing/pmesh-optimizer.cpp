...
// element by element bfgs
   ...
   DenseTensor A1b(dof*dim,dof*dim,NE);
   DenseMatrix A1t(dof*dim,dof*dim),A2t(dof*dim,dof*dim),
               A1A3t(dof*dim,dof*dim),A4A1t(dof*dim,dof*dim);
   Vector ec(dof*dim),er(dof*dim), erksav(dof*dim);
   Vector eyk(dof*dim), esk(dof*dim), eykA1(dof*dim);
   Vector csav;
//
...
int main (int argc, char *argv[])
{
   ...
   else if (solver_type==2) 
   {
    ...
    {
       cout << "GDIteration: rtol = " << newton_rtol << " not achieved."
            << endl;
    }
   delete gdo;
   }
   ...
   {
      bfo = new BFGSSolver(*ir, pfespace, x,
                                       &x0, &remap_gf_init,
                                       &remap_gf, advector);
     if (myid == 0)
      {cout << "The BFGS Optimizer is used." << endl;}
    int start_s=clock();
    bfo->SetPreconditioner(*S);
    bfo->SetMaxIter(newton_iter);
    bfo->SetRelTol(newton_rtol);
    bfo->SetAbsTol(0.0);
    bfo->SetPrintLevel(verbosity_level >= 1 ? 1 : -1);
    bfo->SetOperator(a);
    bfo->Mult(b, x.GetTrueVector());
    x.SetFromTrueVector();
    if (myid == 0 && bfo->GetConverged() == false)
    {
       cout << "BFGSIteration: rtol = " << newton_rtol << " not achieved."
            << endl;
    }
   delete bfo;
   }
