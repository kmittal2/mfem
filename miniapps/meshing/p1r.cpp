...
static uint np, id;
struct pt_data { double x[D], r[D], dist2, ex; uint code, proc, el; };
...
//        cout << i << " " << j << " " << fmesh[0][np] << " " << fmesh[1][np] << " " << ip.x << " " << ip.y << " k10a\n";
        np = np+1;
      }
   }
   }
...
//  std::ifstream infile("randrst.txt");
  std::ifstream infile("randrstRT2d.txt");
  std::string line;
  ...
  iss >> nxyz;
  nxyz = 75;
  if (myid==0) {cout << nxyz*NE << " Points in findpts\n";}
...
// Print Results
   ...
   {
    n = n+1;
    double rerr = abs(pt->r[0] - vrx(it));
    double serr = abs(pt->r[1] - vry(it));
    double merr = max(rerr,serr);
    maxrerr = max(maxrerr,merr);
//    cout << pt->r[0] << " " << pt->r[1] << " " << vrx(it) << " " << vry(it) << " k10n\n"; 
//    cout << n << " " << ie << " " << it << " " <<  maxrerr << " k10abc\n";
//    cout << pt->x[0] << " " << pt->x[1] << " k10ddc\n";
  ++pt;
   }
   }
  MPI_Barrier(MPI_COMM_WORLD);
  double glob_maxrerr;
  MPI_Allreduce(&maxrerr, &glob_maxrerr, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
  cout << "maximum error in rst is: " << maxrerr << " " << glob_maxrerr << " \n";

// Do findpts eval
//  if(myid==0) printf("call findpts_eval\n");
  pt = (pt_data*) testp.ptr;
  start_s=clock();
  findpts_eval(&pt->ex, sizeof(struct pt_data),
                 &pt->code , sizeof(struct pt_data),
                 &pt->proc , sizeof(struct pt_data),
                 &pt->el   , sizeof(struct pt_data),
                 pt->r    , sizeof(struct pt_data),
  ...
  MPI_Barrier(MPI_COMM_WORLD);
  stop_s=clock();
  if (myid==0) {cout << "findpts_eval time taken (sec): " << (stop_s-start_s)/1000000. << endl;}
...
//  cout << pt->x[0] << " " << pt->x[1] << " " << pt->ex[0] << " " << abs(val-pt->ex[0]) << " K10c\n";
  ...
  int glob_nnpt;
  MPI_Allreduce(&nnpt, &glob_nnpt, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
