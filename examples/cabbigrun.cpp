...
int main (int argc, char *argv[])
{
    ...
    const char *mesh_file = "../data/ball5to1.mesh";
    int order = 1;
...
// Do parallel refinements
    ...
    int LNE = pmesh->GetNE();
    int GNE = 0;
    MPI_Allreduce(&LNE, &GNE, 1, MPI_INT, MPI_SUM,MPI_COMM_WORLD);
    if (myid == 0)
    {
        cout << GNE << " total_element_count\n";
    }
    ...
    int ptflag = 1;
    int nptdir = 4; // Order of the quadrature rule.
    ...
    if (myid == 0)
    {
       cout << mesh_poly_deg << " " << rp_levels << " " << nptdir << " deg,rp,nptdir\n";
    }
    ...
    if (combomet == 1)
    {
       ...
       tj2    = new TargetJacobian(TargetJacobian::IDEAL_EQ_SIZE, MPI_COMM_WORLD);
       ...
       c2 = new ConstantCoefficient(0.5);
    ...
    if (myid==0)
    {
        cout << "the number of procs is " << num_procs << " \n";
    }
    if (bndrflag != 1)
    {
